...
class RnntLoss(nn.Module):
    ...
    def forward(self, h, targets, input_lens, target_lens):
        batch_size, n_frames, adjusted_n_tokens, vocab_size = h.shape
        n_tokens = adjusted_n_tokens - 1
        h = self.log_smax(h)
        log_alpha = np.zeros(batch_size, n_frames, adjusted_n_tokens).to(h.device)
        for t in range(n_frames):
            for u in range(adjusted_n_tokens):
                if u == 0:
                    if t == 0:
                    log_alpha[:, t, u] = 0.

                    else: #t > 0
                    log_alpha[:, t, u] = log_alpha[:, t-1, u] + joiner_out[:, t-1, 0, self.blank_idx] 
                    ...
                    log_alpha[:, t, u] = torch.logsumexp(torch.stack([
                        log_alpha[:, t-1, u] + joiner_out[:, t-1, u, self.blank_idx],
                        log_alpha[:, t, u-1] + torch.gather(joiner_out[:, t, u-1], dim=1, index=y[:,u-1].view(-1,1) ).reshape(-1)
                    ]), dim=0)
        log_probs = [log_alpha[b, input_lens[b]-1, target_lens[b]] + h[b, input_lens[b]-1, target_lens[b], self.blank_idx]]
